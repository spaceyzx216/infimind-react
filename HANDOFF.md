...
### 2.17 ⭐ 老浏览器长思考崩溃修复（第五轮）

**症状**：部署后部分浏览器在**深度思考**时崩溃/卡死，集中在 **2018 款 MacBook Air 的 Safari** 与 **360 浏览器**；Edge / Chrome 正常。

**先排除的假设**（都实测过，均不成立）：

| 假设 | 实测结论 |
| --- | --- |
| 产物语法太新，老 Safari 解析不了 | ❌ `target: 'es2015'` 生效，8/8 产物文件可按 **ES2019** 成功解析；`?.` / `??` / `??=` 零残留 |
| React 用的是旧同步渲染 | ❌ 已是 `createRoot`（并发渲染） |
| 部署跑的是 dev server（StrictMode 双渲染） | ❌ nginx 直接服务本地构建的 `dist/`，生产无 StrictMode |

**真正根因：渲染风暴。** 用真实抓取的 SSE 流回放（不是估算）：

| 档位 | SSE 事件 | 旧实现渲染次数 | 旧实现 Markdown 解析 |
| --- | --- | --- | --- |
| 快速 | 5,479 | 5,475 | 2,713 |
| **深度思考** | **13,620**（740KB） | **13,620** | 2,520 |

- 每个事件一次 `setState` → 一次**整页**重渲染（会话列表、证据面板、全部历史 Markdown 都跟着重算）；
- 正文每来一块就 `<ReactMarkdown>{整篇}</ReactMarkdown>` → **O(n²)** 解析；
- 思考面板每帧读 `scrollHeight` 写 `scrollTop`（**强制同步布局**），且文本节点随思考增长到 1.8 万字；
- 深度档事件率约 **66 个/秒**，几乎每帧都有更新 → Chrome 扛得住，老 Safari/360 直接崩。

**修复（四层，缺一不可）**：

| 层 | 手段 | 效果 |
| --- | --- | --- |
| ① 事件合并 | `stream-buffer.js`：增量进缓冲，**每帧至多提交一次** | 只在事件率 <60/秒 时有效，实测仅 −26% |
| ② 提交限频 | 同一模块再加 **100ms 最小提交间隔** | 把渲染次数**钉死**，上游再密也不劣化 |
| ③ 解析节流 | 正文按 200ms 节流成 Markdown 快照 + `AnswerBody` 用 `React.memo` | 解析 −94~96% |
| ④ 布局收敛 | 滚动移到 rAF；`overflow-anchor:none` + `contain:content`；流式期间思考只渲染**尾部 2 万字** | 解耦布局开销与文本长度 |

**实测效果（真实流回放，内容零丢失）**：

| 档位 | React 渲染 | Markdown 解析 |
| --- | --- | --- |
| 快速 | 5,475 → **375**（**−93.2%**） | 2,713 → **97**（−96.4%） |
| 深度思考 | 13,620 → **1,566**（**−88.5%**） | 2,520 → **145**（−94.2%） |

**为什么 ② 不能省**：只做 ①（每帧一次）时深度档仍有约 10,000 次渲染（只 −26%）——因为上游事件率就贴着刷新率。决定老浏览器生死的不是"合并比"，而是**渲染绝对次数**。

**⚠️ 裸 `max-height` 不能限制布局代价**：`max-height:260px` 只裁剪**绘制**，浏览器仍要为整个文本节点算出行盒。4.7 万字的节点每帧重排一次正是 Safari 卡死处，所以必须在流式期间就把进入 DOM 的字数限住。

**顺带修掉的 360 浏览器白屏问题**：`dist/index.html` 只有 `<script type="module">`，而**双核浏览器可能默认落进 IE 兼容模式**，IE 内核不认识 `type="module"` → 整页白屏（与"崩溃"是两回事，但用户都描述成"打不开/坏了"）。已在 `vite.config.js` 的 HTML 生成器里注入 `<meta http-equiv="X-UA-Compatible" content="IE=edge">` 强制走最高内核。

> ⚠️ `index.html` 是**构建产物**（在 `.gitignore` 里，且插件原先仅在文件不存在时生成）。
> 改 `index.html` 本身**不会生效**——必须改 `vite.config.js` 的 `generateHTMLPlugin()`。
> 现已改为"缺失或版本过期时补写"且幂等。

### 2.18 《公司法》收录 + 版本判定加固（第五轮）

**背景**：用工咨询在回答企业治理相关的用工问题（董事会/监事会职工代表、清算时职工债权顺位）时会引用《公司法》，但白名单未收录 → 全部标注「未收录，需人工核实」。

**收录**（`LAW_WHITELIST_SEED`，用户提供官方全文 doc，已逐条核对）：

| 字段 | 值 |
| --- | --- |
| 版本 | **2023年修订**（2023-12-29 十四届全国人大常委会第七次会议**第二次修订**） |
| 文号 | 中华人民共和国主席令第十五号 |
| 施行 | **2024-07-01**，共 15 章 266 条 |
| review_status | `verified` |

别名含 `公司法`、`新公司法`，否则模型写「《公司法》第X条」会落到未收录。

**用工相关条款索引**（已写入 note，随提示词一起注入模型）：
第16条（签订劳动合同/社保/劳动保护/职业培训）、第17条（工会、集体合同、民主管理、制定重要规章制度须听取工会与职工意见）、第20条（利益相关者与社会责任）、第68条（**300人以上**有限责任公司的董事会职工代表）、第76/130条（监事会职工代表**不低于三分之一**）、第121条（审计委员会职工代表）、第173条（国有独资公司董事会职工代表）、第236条（**清算时职工工资、社保费用、法定补偿金优先清偿**）。

**⭐ 顺带修掉一个防幻觉漏报**（`citation-verifier.js`）：

原版本判定用 `(versionLabel + effectiveFrom + effectiveTo).includes(年份)`。**日期字符串里天然带年份**，于是：

| 模型标注 | 修复前 | 修复后 |
| --- | --- | --- |
| 《公司法》**2024年修订版**（**不存在**） | ✅ 误放行 ← `effectiveFrom='2024-07-01'` 含 "2024" | ❌ 拦截 |
| 《公司法》2018年修订版（旧版） | ❌ 拦截 | ❌ 拦截 |
| 《公司法》2023年修订（正确） | ✅ 放行 | ✅ 放行 |

《公司法》恰好"修订年（2023）≠ 施行年（2024）"，所以正好命中这个漏洞——**这是本功能最该拦住的一类幻觉**（见 §1 的核心命题）。同类风险对任何"施行年份≠版本年份"的法规都成立。

修复方式：改为与 `versionLabel` 做**归一化后的双向包含**（不再看日期），并归一"2023修订/2012修正"这类省略「年」的写法。同时放宽 `VERSION_HINT_PATTERN` 的「年」为可选——原先识别不了「2023修订」，会**跳过**版本校验。

**实测（13 条引用全部通过）**：真实咨询中模型主动写出「注：不存在"2024年修订版""2018年修订版"，2018-10-26 为旧版修正」——note 注入正在有效抑制幻觉。回归 **101 通过 / 0 失败**（新增 4 项）。
...
| # | 事项 | 影响 | 处理方式 |
| --- | --- | --- | --- |
| 1 | **法规白名单 15/20 条待人工核对** | 未核对条目的引用只标注「已收录（待复核）」，可信度打折 | `npm run verify:laws` 拿清单，核对后 `npm run verify:laws -- --confirm "法规全称" 姓名` |
...
| 事项 | 说明 |
| --- | --- |
| **老浏览器渲染（第五轮已修，待真机验收）** | 2018 MacBook Air 的 Safari / 360 浏览器在**长思考**时崩溃。根因是渲染风暴（实测深度档 13,620 个 SSE 事件 → 13,620 次完整渲染 + 2,520 次整篇 Markdown 重解析）。已按帧 + 最小间隔节流（渲染 **−88.5%**、解析 **−94.2%**），但**尚未在真机复验**，且新增的 `X-UA-Compatible` 需重新部署 `dist/` 才生效 |
...
| 文件 | 职责 |
| --- | --- |
| `src/pages/LaborConsultPage.jsx` / `.css` | 咨询页（含 `ReasoningPanel` 思考折叠面板、侧边栏「标题 + 相对时间」两行条目） |
| `src/utils/file-selection.js` | **附件选择共享逻辑**（三个工作台共用，含格式白名单） |
| `src/utils/conversation-title.js` | **会话标题启发式**（前端与服务端共用同一份；含"仅附件"合成文案识别） |
| `src/utils/relative-time.js` | **相对时间格式化**（分钟/小时/天/月/年，向下取整不出越界单位） |
...
| `LABOR_TITLE_REFINE` | 默认开 | 设 `off` 关闭会话标题 LLM 提炼（保留本地启发式标题，省一次约 0.5~1.0s 的后台调用） |
...
| 法规白名单 | 20 条（已核对 5 / 待核对 15；含《公司法》2023修订） | `npm run verify:laws` |
